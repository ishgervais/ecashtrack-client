...
export default function TempBookingAmountChart(props: { year: number }): JSX.Element {
	let year = props.year
	...
	function getTotalAmountForBookingsInMonth(mon: string) {
		...
		for (let k = 0; k < data?.length; k++) {
			...
			if (dbMonth === getMonthInt(mon)) {
				...
				totalEstimatedPayment += data[k].estimated_payment
			}


		}
		return internationalNumberFormat.format(totalAmount)
	...
	function getBookingsInMonth(mon: string) {
		...
		return internationalNumberFormat.format(totalCount)
	}
	const data = [
		{
			month: "Jan",
